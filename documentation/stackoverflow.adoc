...
We started with the Jaccard Similarity which is based on the intersection and the total size of two sets:
...
We can run Jaccard Similarity in Neo4j as a function. 
A simple version as an exercise:
...
Finding the Jaccard similarity between tags:
